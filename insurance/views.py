...
@login_required(login_url='adminlogin')
def admin_view_pending_customers_view(request):
    customers= CMODEL.Customer.objects.filter(status='Pending')
    return render(request,'insurance/admin_view_pending_customers.html',{'customers':customers})
...
def approve_customer_view(request,pk):
    ...
    customer.save()
    return redirect('admin-view-pending-customers')
...
def admin_add_policy_view(request):
    ...
    if request.method=='POST':
        ...
        if policyForm.is_valid():
            category_name = policyForm.cleaned_data['category_name']
            category, created = models.Category.objects.get_or_create(category_name=category_name)
...
@login_required(login_url='adminlogin')
def terminate_policy_view(request, pk):
    policy_record = models.PolicyRecord.objects.get(id=pk)
    policy_record.status = 'Terminated'
    policy_record.save()
    return redirect('admin-view-policy-holder')
