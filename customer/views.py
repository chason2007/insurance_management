...
def profile_view(request):
    ...
    if request.method == 'POST':
        user_form = forms.ProfileUpdateForm(request.POST, instance=user)
        customer_form = forms.CustomerProfileForm(request.POST, request.FILES, instance=customer)
        if user_form.is_valid() and customer_form.is_valid():
            user_form.save()
            customer_form.save()
            return redirect('profile')
    else:
        user_form = forms.ProfileUpdateForm(instance=user)
        customer_form = forms.CustomerProfileForm(instance=customer)
